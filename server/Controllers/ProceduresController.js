...
class ProceduresController {
    async getAvailableProcedures(req, res) {
        try {
            const { mastersProceduresId } = req.body;
            const availableProcedures =
                await ProceduresService.getAvailableProcedures(
                    mastersProceduresId,
                );
            return res.json(availableProcedures);
        } catch (e) {
